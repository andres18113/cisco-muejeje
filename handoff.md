...
The causal correction changes observer granularity, not authority. All 14
typed Floor2 bindings are dispatched once, then one grouped logical table read
must simultaneously match each new directory index, extension, and measured
MAC. A complete mismatch fails; a complete absence retains the single
absence-driven reconciliation path; incomplete or unattributed tables fail
closed after the same one safe read-only retry. Registration, access FWD,
endpoint IPv4, DHCP binding, and SCCP remain cumulative for all 35 phones.
