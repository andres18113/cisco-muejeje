...
The next correction reuses one shared policy already proven by Voice: at most
one additional read-only query is permitted only when the first result proves
executed, fresh, incomplete, pager-truncated, continuation-failed, and uniquely
attributed to the expected device. A mutation is never retried. The
control-plane result retains a two-attempt `ConvergenceReport` with first/final
pager authority dimensions so the next LIVE can prove whether the policy
engaged and whether it closed the observer.
