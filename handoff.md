...
The correction is limited to `parse_show_ephone()`: it accepts spaces or tabs
before a strict ephone header, just as the prior measured correction accepts
indentation before `IP:`. MAC, registration, extension, and line-state parsing
remain fail closed. The exact pager-boundary shape has a fail-first regression
test. No topology, Voice lifecycle, verification threshold, mutation order, or
capacity was changed. A canonical production LIVE is still required.
