...
| 4 | **SATISFIED.** Twelve declared foundations (9 `l3_interface`, 3 `link`), statuses derived by `derive_foundational_statuses` from executed `apply_configuration` results only — no parameter can supply one. `ControlPlaneApplicator` decided on them before touching the runtime and admitted E9. This is the row the harness satisfied with a comprehension. **Hashes: none, and correctly none** — only `kind="security"` requirements declare a `source_hash`, a routing-only plan declares zero, so `derive_foundational_hashes` returns `{}` and the gate compares a hash only where one is declared. Verified against this exact plan. The row's "statuses and hashes" is satisfied by the statuses deciding it and the hash comparison being a declared no-op, not by hashes being produced. |
