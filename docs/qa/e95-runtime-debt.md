...
| PoE | Capability discovery reads `getPower()` and `isPowerOn()` separately, requires strict complete homogeneous observations, and preserves missing, malformed, mixed, wrong-model, or wrong-build evidence as unknown. Powered-device delivery remains a separate unobserved field. | [OK] `FIXED_AND_VERIFIED` for the hardware-capability claim on Packet Tracer `9.0.1.0858` / `3560-24PS`: 24 fresh unlinked FastEthernet ports returned `true/true`; the `2950T-24` negative control returned `false/false`. The registered exact-build snapshot is `SUPPORTED` / `VERIFIED` with `observed_value=24` and `object_state`; no model-name inference. Powered-device delivery is not claimed. |
