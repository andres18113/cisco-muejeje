...
## 11. S1 correction and coexistence record

This section is the proportional L-risk change brief for the independent review
of S1 commit `0bddc9a5203584b14869bdd9cabb071f25d528e1` (tree
`4c46e8cf5838a2b26f3cc99d51ef724ea196bd84`). The review disposition is
`REQUIRES_CHANGES`; it authorizes only the causal corrections below, historical
source packaging, and offline verification. It does not accept S1, authorize
Packet Tracer contact, promote a capability, merge, push, or begin another
slice. The authoritative base remains `cisco/main` at
`6263344e31ba3b0de6539d652f2cd06fc73a3562` as re-resolved in this checkout.

The review and its historical inputs are preserved under
[`docs/reference/server-pt/`](../../reference/server-pt/README.md). Those files
are immutable inputs; this section is the maintained implementation record.

### 11.1 Problem and intended outcome

The S1 product path does not yet enforce the designed contract at its real
composition boundary. It can select transport before process admission, let
different collaborators select different channels, omit observed provenance,
validate excluded configuration work, leave retained-result reuse disconnected,
promote the public status above the E6 result, lose release and coverage facts,
and leak real filesystem errors outside the typed persistence boundary.

The intended outcome is one fail-closed product path that admits before bridge
contact, binds all reads and writes to one admitted session, validates only the
required E5/E6 closure while preserving the complete plan identity, safely
reuses freshly verified prior results, reports monotone E6/client/release facts,
and stops new effects when durable stage control is lost.

### 11.2 Scope and explicit exclusions

The correction includes:

- the existing `pt_apply_enterprise_services` construction and its bridge,
  environment, source-tree, store, E5, and E6 collaborators;
- active/retained/excluded closure admission for the approved static,
  same-site, same-segment Server-PT-to-PC-PT path;
- product-connected A10 retention with bounded history lookup and fresh
  verification;
- public aggregation, complete selected-pair coverage, and projection of the
  accepted S0 release contract;
- typed filesystem/identity containment across begin, load, advance, write,
  replace, read, and completion;
- deterministic coexistence and scale measurements over synthetic offline
  inventories and 2, 20, 200, and 1000 reporting clients; and
- exact archival packaging and documentation navigation.

It excludes SMTP, POP3, Server-PT DHCP, routed services, general inter-switch
paths, HTTPS expansion, IoT, a new engine protocol, cross-process concurrency,
LIVE Packet Tracer work, and line-count-driven registry extraction. The
single-writer discipline remains in force. Synthetic scale tests are not a
claim that a physical 1000-host topology is supported.

### 11.3 Requirements and acceptance criteria

| Requirement | Acceptance criterion |
| --- | --- |
| S1-01 production admission, binding, and provenance | Real `register_tools` construction refuses malformed input and `TEST_PROCESS` with zero health, heartbeat, or bridge calls; after admission, inventory, directed reads, E5 dispatch, typed E6 dispatch, and finalization remain on the one selected channel even if the picker changes; typed dispatch outcomes and distinct manifest/current/source-tree provenance cases survive unchanged and secrets do not enter records |
| S1-02 effect-scope admission | One active/retained/excluded partition drives identity, dependency, interface, orientation, capability, path, and verification admission; unrelated excluded missing devices, serial actions, and foreign deployment data neither execute nor contaminate the selected result; missing or ambiguous selected targets, excluded required dependencies, DHCP clients, routed paths, and unsupported inter-switch paths refuse before E5 effects; the no-exclusion legacy path remains unchanged |
| S1-03 retained reuse | Two calls through the real coordinator and temporary store freshly verify eligible retained E5 rows and do not redispatch them; environment/plan/drift changes, incomplete or failed rows, corrupt records, and newer interrupted or uncertain attempts refuse retention without resurrecting an older success; persisted mutated, retained, and excluded identifiers match the decision |
| S1-04 monotone aggregation and coverage | Actual E6 accepted-unknown, contradiction, critical failure, full success, and DNS functional success with residue-only `UNKNOWN` remain monotone in public JSON, typed result, and stored record; every selected service/client pair remains visible when excluded, blocked, halted, or not attempted; actual S0 release facts survive first-class and missing release evidence never becomes success |
| S1-05 durability and identity | Real failures at base/deployment directory creation, temporary creation/write/replace, read, transition, and completion become typed safe results; no new mutation begins after durability loss; earlier facts and the last durable stage remain available; primary and secondary failures remain distinguishable; run/deployment identity collisions cannot overwrite or masquerade |
| S1-COEX | Existing CP-SCALE, voice, control-plane, services-opt-out composition, E5 defaults, S0 facts, namespace isolation, manifest identity, and both Node harnesses remain green; bounded service invocations touch only selected identities, required prerequisites, and owned temporaries |
| S1-SCALE | Aggregation and retained-history work is indexed or bounded, with deterministic operation-count tests at 2/20/200/1000 clients and increasingly large unrelated inventories; budgets are derived from measured offline behavior and existing transport limits, not presented as LIVE capacity |
| S1-DOC | Every historical source copy matches its source SHA-256 and byte count, staged Git blobs are verified explicitly for conversion effects, the source manifest is valid, MkDocs navigation reaches the archive, and namespace inventory remains closed |

### 11.4 Architectural impact and affected contracts

The adapter may gain one small lazy session binding/factory at the existing
composition boundary. It must reuse the current bridge and typed transport and
must describe each bound callable's channel, identity, return, and error
semantics. It is not a second registry, a new protocol, or a public bypass.
Legacy callers retain their signatures and service composition remains opt-in.

The application layer owns the single E5 closure partition, path admission,
A10 decision, monotone aggregation, and complete selected-pair projection. E5
and E6 continue to use their existing applicators and canonical
`decide_mutation` semantics. Infrastructure owns bounded record discovery,
atomic persistence, and concrete backend observations. Domain records continue
to carry facts rather than granting execution merely because they deserialize.

### 11.5 Invariants

- Parsing and `ImportIsolationPreflight` precede every external health,
  heartbeat, inventory, bridge, store-construction, or mutation action that is
  not required to record an already-admitted run.
- Required identity and provenance are observed and exact; missing values are
  limitations or refusals, never guessed values. A manifest is not a runtime
  observation and early unbound refusals do not pretend they executed a SHA.
- One admitted channel is used for every E5/E6 read and write in a run. A lost
  legacy boolean does not prove that no effect occurred, and typed E6 dispatch
  facts are not reconstructed from text.
- The complete plan identity is stable. Active, retained, and excluded action
  sets are disjoint; required dependencies cannot be excluded or silently
  disappear; no retained row is fabricated.
- E6 uncertainty, contradiction, and critical failure are sticky at the
  product aggregate. Independently verified client facts and residue-only
  unknowns remain visible without erasing the aggregate constraint.
- A write-ahead stage transition is durable before its next mutation. Local
  pre-dispatch refusal proves only that the refused call was not dispatched;
  it cannot erase earlier uncertain effects.
- Historical evidence and archived source bytes are immutable. Offline tests
  and synthetic scale measurements never establish LIVE support.

### 11.6 Test design and verification levels

Behavioral findings are reproduced RED against the real affected component
before their causal fix. Unit tests cover pure closure, aggregation, release
projection, bounded history, and operation counts. Integration tests exercise
the real coordinator/applicators/store with controlled ports. Product-surface
tests exercise real `register_tools` construction and controlled external
health/transport seams. System regression covers CP-SCALE, voice, control
plane, manifests, namespace isolation, services opt-out, and both Node
harnesses. LIVE acceptance is non-applicable because it is explicitly not
authorized; exact-SHA CI remains pending unless a later push is authorized.

Verification proceeds focused RED/GREEN, affected suites, full offline suite,
both Node harnesses, namespace inventory, MkDocs, whitespace, self-review of
the S1-only and correction-only diffs, and a clean exact-commit quality gate
against `cisco/main`. Results, deviations, remaining unmeasured gates, commit,
and tree are appended to this section only after they are observed.

### 11.7 Implementation and causal RED results

The correction kept the existing product path and introduced no second bridge,
wire protocol, registry, or general tool-family extraction.

- `service_tools.py` now gives the use case lazy manifest and run-record
  factories plus one post-A4 `ServiceInvocationBinding`. The binding fixes one
  channel for target-filtered inventory, E5 dispatch, endpoint reads, typed E6
  dispatch, and final observations. It observes the active file's current
  backend version and the checkout's source SHA/dirty state instead of copying
  caller or manifest values into runtime provenance.
- `tool_registry.py` preserves the existing bridge and both transports. Its
  target-filtered inventory scans device names to retain missing/duplicate
  detection but enumerates ports only for requested manifest names. HTTP and
  file typed dispatch outcomes reach E6 without passing through the legacy
  string-returning seam.
- `apply_enterprise_services.py` resolves optional eligibility before runtime
  identity reads; enforces the static Server-PT/static PC-PT, same-site,
  same-segment, single-access-switch path; connects A10 retention; indexes
  client/service/expectation rows; projects the accepted S0 release DTO; and
  caps public status monotonically at the E6 aggregate.
- `apply_configuration.py` validates targets, interfaces, serial orientation,
  capabilities, and verification prerequisites for the active/retained
  partition. With no explicit partition it preserves the whole-plan contract.
- `ServiceRunRecordStore` is construction-side-effect-free, validates internal
  identity against its path, refuses begin collisions, contains filesystem
  failures, scans at most 257 history paths for a 256-record budget, rejects
  failed retained rows, and blocks replay after a newer interrupted or
  effect-uncertain attempt for the same configuration even across environment
  changes. The run-id generator now belongs to the record model rather than an
  application-to-persistence import.

Measured RED preceded each causal correction:

| Finding | Reproduced RED | Causal GREEN |
| --- | --- | --- |
| S1-01 | The production surface tests made the actual health picker observable: malformed input and `TEST_PROCESS` each failed because one health read had already occurred | Both now make zero HTTP/file health calls through actual `register_tools`; a changing picker is called once and every collaborator records the admitted channel; parameterized `NOT_SUBMITTED`, `REJECTED`, and `ACCEPTANCE_UNKNOWN` facts reach E6 unchanged |
| S1-02 | An explicitly bounded configuration failed on an unrelated missing device; DHCP product input stopped only as a generic composition failure; path predicates accepted missing foundations and multi-switch same-segment countermodels | The bounded partition ignores unrelated excluded identities while missing/ambiguous dependencies, DHCP, non-PC clients, foreign/routed paths, missing access foundations, and multi-switch paths refuse before E5 |
| S1-03 | A second real-coordinator call redispatched every E5 row, and a newer uncertain call did not block another replay | The second identical call freshly rereads prerequisites, records retained IDs, and dispatches no retained row; newer uncertain exact or changed-environment records produce `RETAINED_RESULT_INVALID` |
| S1-04 | Optional client/service pairs were absent; successful client reads promoted E6 critical contradiction or typed dispatch uncertainty; production `observed.released` facts produced no `releases[]` rows | Coverage includes every selected pair and halted work; E6 failure/uncertainty is monotone; residue-only unknown remains independently visible; successful, unresolved, and malformed release facts survive response and storage |
| S1-05 | Constructor/mkdir, temporary create/write, identity collision, internal identity mismatch, failed retained rows, and newer uncertainty tests failed on concrete filesystem/store behavior | All filesystem phases are typed, begin is create-only, advance requires the existing identity, last durable stage survives secondary failure, and no later effect begins after a failed transition |

### 11.8 Coexistence, scale, and verification results

The offline workload bounds are explicit: intent JSON is at most 1 MiB,
matching the existing authenticated bridge body ceiling; the public result is
bounded to 1000 reporting clients and 5000 client-check rows, the measured
current DNS+HTTP shape of five checks per client; retained lookup admits at
most 256 records; and the existing runtime keeps its bounded DNS/HTTP and
correlated transport timeouts. These limits control local work and do not state
Packet Tracer capacity.

Deterministic tests at 2, 20, 200, and 1000 clients measured one expectation
index operation per check, one membership operation per selected client, one
pair assembly per client/service pair, and one row lookup per check. Separate
2/20/200/1000 unrelated-inventory tests kept the selected seven-action E5
closure and touched no unrelated runtime identity. The target-filtered
production inventory still scans names for ambiguity but no longer reads every
unrelated device's ports.

| Verification | Observed result |
| --- | --- |
| Causal/affected S1 set | **330 passed** |
| Dedicated CP-SCALE/voice/control-plane coexistence set | **390 passed**; the final full suite covers the same files after later admission hardening |
| Full offline suite | **5972 passed, 3 skipped, 3 warnings** in 382.28 s |
| Both S0 Node harnesses | **60 passed** in the explicit harness run; also included in the final full suite |
| Provisional quality gate | `cisco/main` and merge base `6263344e31ba3b0de6539d652f2cd06fc73a3562`; 47 changed Python files; zero mechanical exemptions; all checks passed |
| Namespace inventory | zero active legacy imports, zero active legacy string references, and zero unreviewed inert mentions both before and after staging |
| MkDocs | Built successfully; only the two pre-existing missing `handoff.md` warnings under `docs/reference/cp-scale/` |
| Historical archive | Seven source, destination, and staged Git blob byte sequences matched their recorded byte counts and SHA-256 hashes; `.gitattributes` preserves historical Markdown as `-text -diff` |
| Whitespace | Worktree and S1/base diff checks passed |

The three skips are not S1 coverage: Windows symlink creation is unavailable
to this account, and two retained qualification artifacts are absent from this
checkout. The three warnings are the pre-existing pytest deprecation warning
for class-scoped instance-method fixtures.

Deviations and remaining limits:

1. The companion overlay ZIP named by the review was not present in Downloads.
   The seven individually supplied sources were copied without removing the
   originals and are governed by the same byte/hash manifest.
2. No trustworthy current extension-version observation exists on this path.
   A manifest requiring one therefore yields typed
   `RUNTIME_PROVENANCE_UNAVAILABLE`; it is not copied from the manifest and is
   not promoted.
3. D-8 registry modularization remains deferred. The correction adds only the
   narrow session binding and target-filtered inventory required by S1-01.
4. No Packet Tracer, product bridge, CP-LIVE, concurrent-writer, routed-service,
   or physical scale execution occurred. All capability provenance remains
   documentary and LIVE usability remains unverified.
5. Exact-SHA CI is pending because push was not authorized. Local commit/tree,
