# Server-PT services for PC-PT clients: architecture and implementation plan (revision 2.2)

Status: **READY_FOR_REVIEW**. Not approved and not implementation-ready without
the independent approval of the S0 contract in section 7.1 and the gates named
in sections 4.6, 5.8 and 7.3. Risk class **L**.
...
`docs/engineering/standards.md`: it records design before implementation,
implements nothing, modifies no repository file and runs no Packet Tracer. It
is self-contained: a copy committed under `docs/engineering/change-briefs/`
needs no other file. Revision 2.2 answers the correction review of revision
2.1 (`Fable_S0_rev21_Two_Finding_Correction.md`, findings C1 and C2) and
changes nothing outside those findings and their dependent clauses;
revision 2.1 answered B1 to B3 and G1 to G4; revision 2 answered SR-01 to
SR-10. Dispositions are in `Server-PT-Services-Review-Resolution-rev2.2.md`.
...
| Instruction loading | `/context` in this session lists `CLAUDE.md`, `AGENTS.md` and `docs/engineering/standards.md` under Memory files; the implementer repeats the check in their fresh session | observed once; pending per implementer |
...
The standalone copy is `Server-PT-Services-Implementer-Prompt-S0-rev2.2.md`;
both copies are identical after outer whitespace normalization.
