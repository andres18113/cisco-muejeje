...
_LEGACY_ACL_ENTRYPOINT = "packet_tracer_mcp.adapters.mcp.tool_registry.pt_apply_acl"
...
def _physical_policy(
    ...
    return _named_policy(
        MutationSurface.PHYSICAL,
        family,
        _PHYSICAL_ENTRYPOINT + method,
        classification,
        basis,
        containment,
        evidence,
...
PRODUCT_MUTATION_REPLAY_REGISTRY: tuple[MutationReplayPolicy, ...] = (
    ...
    _named_policy(
        MutationSurface.LEGACY_RAW,
        "pt_apply_acl (ACLPlan)",
...
def _validate_registry() -> None:
    ...
    if len(families) != len(set(families)):
        raise RuntimeError(
            "Product mutation replay registry contains duplicate families."
        )
    action_types = [
        item.action_type
    ...
    for surface, surface_expected in expected.items():
        ...
        if surface_actual != surface_expected:
            missing = sorted(
                item.__name__ for item in surface_expected - surface_actual
            )
            unexpected = sorted(
                item.__name__ for item in surface_actual - surface_expected
            )
            raise RuntimeError(
